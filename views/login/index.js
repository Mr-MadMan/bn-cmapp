...
export default function Login() {
  ...
  const signUp = () => {
    if (!info.account && !info.account.trim()) {
      return Alert.alert('Account can not be empty')
    }
    if (!info.password && !info.password.trim()) {
      return Alert.alert('Password can not be empty')
    }
  }
  ...
  return (
    <View style={styles.container}>
      ...
      <Button onPress={signUp} buttonStyle={Object.assign({}, styles.btnStyle, { marginBottom: 10, backgroundColor: '#2089dc' })}>Create account</Button>
